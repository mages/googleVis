...
The Google Visualisation API~\cite{GoogleVisApi}
...
However, before you use the API you should read the \href{https://developers.google.com/terms/}{Google
  Visualization API Terms of Service}\cite{GoogleTerms}. 
...
\subsection{Combining charts with \texttt{gvisMerge}}
...
% \subsection{Using \googleVis with \Rook}
% 
% <<eval=FALSE>>=
% require(Rook)
% require(googleVis)
...
% 
% my.app <- function(env){
%  ## Start with a table and allow the user to upload another csv-file
%   req <- Request$new(env)
%   res <- Response$new()
% 
%   ## Provide some data to start
%   data <- Exports[,1:2]
%   if (!is.null(req$POST())) {
...
%   res$write(paste(tbl$html$chart, collapse=''))
% 
%   res$write('
...
\section{Using \googleVis with \knitr}
Using \googleVis with \knitr~\cite{knitr} is a convient way of creating
interactive reproducible reports. The approach taken by
\knitr is similar to Sweave, in that you can combine code with text, see
the \knitr home page\footnote{\url{http://yihui.name/knitr/}} for more 
details. However, \knitr can also export to HTML, which is required to 
embed \googleVis charts.
...
```
\end{verbatim}
Further examples have been published in a blog post: 
