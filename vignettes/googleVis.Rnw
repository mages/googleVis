...
framework is the newest and certainly the easiest option to 
...
Shiny\footnote{\url{http://www.rstudio.com/shiny/}} is a package from
RStudio that makes it incredibly easy to build interactive web
applications with R.  
...
similar way to other plotting functions. Note that \shiny of version
$\ge$ 0.3.1.99 is required.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\textwidth]{shiny.png}
...
<<eval=FALSE>>=
library(shiny) ## Version >= 0.3.1.99 required
...
\googleVis charts on mages' blog:
...
\url{http://lamages.blogspot.co.uk/2012/05/interactive-html-presentation-with-r.html}. The
basic idea is that you create a Markdown file with \knitr and you
post-process the output with pandoc into a slidy HTML presentation. 
\begin{figure}[h]
  \centering
