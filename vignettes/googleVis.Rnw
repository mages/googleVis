...
Visualisation API, one year later.  
...
\googleVis package~\cite{googleVis}, \cite{RJournal:2011-2:GesmannCastillo}. 
...
package helps, as it transforms R data frames into
...
Figure~\ref{demos} for some examples. All available charts types are
listed on the
\href{http://code.google.com/p/google-motion-charts-with-r/wiki/GadgetExamples}{project
  site}\footnote{http://code.google.com/p/google-motion-charts-with-r/wiki/GadgetExamples}. 
...
package, see also Figures~\ref{demos}.
...
such as the chart. This is particularly helpful if the package functions
...
Please note that this feature of \texttt{plot.gis} was introduced
...
in a interactive way, so you set \texttt{options(gvis.plot.tag=NULL)}
...
Here is another example setting the  colour axis of a geo chart, see Figure~\ref{geochart}:
<<eval=FALSE>>=
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=0.8\textwidth]{gvisGeoChart.png}
\caption{
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.5\textwidth]{Apostrophe.png}
...
and upload her/his own CSV-file, see , see Figure~\ref{fig:Rook}. 

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\textwidth]{Rook.png}
...
approach taken by \knitr is similar to Sweave, in that you can combine
...
to set the chunk option results to \texttt{'asis'} and print the chart 
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.7\textwidth]{MarkdownExample.png}
...
As a result you can use the \texttt{options()} command as a switch in your \knitr
file to switch between as interactive mode, where you are likely to
...
The JavaScript library deck.js~\cite{deck.js}, a j{Q}uery
...
An alternative to deck.js is 
slidy\footnote{\url{http://www.w3.org/Talks/Tools/Slidy2/}} 
in combination with \knitr and 
pandoc\footnote{\url{http://johnmacfarlane.net/pandoc/}}, see the following blog post for more details:
\url{http://lamages.blogspot.co.uk/2012/05/interactive-html-presentation-with-r.html}. The basic idea is that to create a Markdown file with \knitr and to post-process the output with pandoc into a slidy HTML presentation.
\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\textwidth]{CambR_googleVis_29_May_2012.png}
  \caption{\googleVis presentation with slidy:
    \url{http://lamages.blogspot.co.uk/2012/05/interactive-html-presentation-with-r.html}} 
  \label{fig:slidy}
\end{figure}
...
\subsection{Can use googleVis output in PDF files?}
No, not directly. 
...
The colours are set by the Google Chart Tools API and cannot changed
by the user.
...
However, you can upload your data to a Google Fusion Table link with a
second table which defines the icons. See the Google Fusion Table
...
Apparently there is little appetite at Google to migrate move motion
charts to HTML5 or other technologies, see the following Google
Visualisation API news group post: \url{https://groups.google.com/forum/?fromgroups=#!topic/google-visualization-api/3dn3zlQMcXI}
%{https://groups.google.com/\-forum/\-?fromgroups=\-#!topic/\-google-visualization-api/\-3dn3zlQMcXI}.
