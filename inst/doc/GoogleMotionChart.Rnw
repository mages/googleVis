...
\author{Markus Gesmann\\markus.gesmann@gmail.com\\Diego de Castillo\\decastillo@gmail.com}
...
\begin{abstract}
  ...
  Visulisation Object.
  The output is html code, which when run on a web server, generates a
  dynamic flash based bubble chart to explore several indicators over
  time.
\end{abstract}

\section{Motivation}
In 2006 Hans Rosling presented at TED~\cite{HansRoslingTedTalk} about
...
had developed animated bubble charts.
His inspiring talk popularised the idea of interactive charts and
eventually the software behind Gapminder was bought by Google and
integrated as motion charts into their Visualisation
API~\cite{GoogleMotionChartApi}.

At the 2010 R/Rmetrics R/Rmetrics Workshop on Computational Finance
and Financial Engineering presented Sebasti\'{a}n P\'{e}rez
Saaibi~\cite{Saaibi2010} the idea to link motion charts with R using
the R.rsp package~\cite{Rsp}.
...
talk~\cite{HansRoslingTedTalk} about social and economic developments in the world, 
See also~\cite{Gapminder}.
