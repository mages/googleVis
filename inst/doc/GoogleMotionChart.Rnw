...
\begin{abstract}
  The \texttt{GoogleMotionChart} package provides an interface between R and the Google Visualisation API.
  The purpose of the package is to allow the user to transform data stored in a R \texttt{data.frame} structure into a Google Motion Chart.
...
DataTable~\cite{DataTable}, and this is where this package helps.
...
\end{figure}
\section{Example}
The package comes with an example data set called \texttt{Fruits}.
<<>>=
require(GoogleMotionChart)
data(Fruits)
Fruits
@
...
<<>>=
MotionChart(Fruits[,-7], "Fruit", "Year") ## exclude Date column
@
...
Further examples are available via demos. See \texttt{demo('GoogleMotionChart')} for a list of demos files.
