...
\href{http://flare.prefuse.org/}{FLARE (ActionScript)} \cite{Flare} to
name but a few.
...
\begin{figure}%%[!ht]
\begin{center}
\includegraphics[width=\textwidth]{googleVisDemoPlots.png}
\caption{
  Screenshot of some of the outputs of \texttt{demo(googleVis)} with 
  \texttt{gvisMotionChart, gvisAnnotationTimeLine,  gvisMap,
    gvisGeoMap,  gvisTable, gvisTreeMap, } from  top left to bottom
  right.   }\label{demos}
\end{center}
\end{figure}
...
folder). Please contact your system administrator initially if you need help. 
...
Figure~\ref{MotionChartGUI}. It works analogue for the other
...
The output of a \googleVis function is a list of lists
containing information about the chart type, chart id and the html
code in a  sub-list with header, chart, caption and footer.

The idea behind this concept is that users can get on one hand side a
...
(\texttt{plot.gvis}) functions exist to ease the handle of such objects. 
...
heave used also \texttt{'Date'} instead of \texttt{'Year'}. 
...
M$chartid
@ 
The html output list is then split into header, chart, caption and
...
two rsp-files to provide a  simple layout framework.
<<>>=
cat(M$html$header)
@ 
...
plot(M)
@
The plot method for \texttt{gvis} object will automatically create by
default a rsp-file in the \texttt{rsp/myAnalysis} folder of the \googleVis package
using the type and chart id information of the object, and it will
display the output using the local web server of the \rsp package.
