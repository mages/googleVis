...
as a named character vector in the \texttt{chart} item of the html list. This
...
for more details. The chart is made up of several JavaScript and 
HTML statements. Please notice that the JavaScript functions are 
...
@
Similarly you can also access also components of the chart, e.g.
<<>>=
