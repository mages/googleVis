...
\caption{
  ...
  \texttt{gvisMotionChart, gvisMap,  gvisTable, gvisTreeMap,  gvisGeoMap} from
...
may want to install the packages locally (e.g. your home
...
default a rsp-file in the \texttt{myAnalysis} folder of the \googleVis package
...
The following R-command will show the path to this folder:
<<eval=FALSE>>=
system.file(file.path("rsp", "myAnalysis"), package="googleVis")
@ 
