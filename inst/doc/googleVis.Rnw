...
development of the \googleVis package~\cite{googleVis}.  
...
json~\cite{json} objects as the basis for the DataTable.
...
\item The data to visualise as a \texttt{DataTable} 
...
To display the visualisation, the html-page it must be loaded from a
web server in a browser with internet connection and Flash; will not
...
interactive chart from the Google Visualisation API we have to
generate html code with links to Google javascript functions, the data
formated as a \texttt{DataTable} and further options; and to display
the chart we need a web server. 
...
tasks. It works analog for the other APIs.
As an example we will show how to generate a motion chart as in 
Figure~\ref{MotionChartGUI}. Further examples are covered in the demos
of the \googleVis package, see also Figure~\ref{demos}.
...
\subsection{\googleVis design principals}
