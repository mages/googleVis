...
However, before you use the API you should read the \href{http://code.google.com/apis/terms/index.html}{Google
...
M <- gvisMerge(gvisMerge(gvisMerge(MC, ATL, TRUE), gvisMerge(Map, Geo, TRUE)), gvisMerge(Table, Tree, TRUE))
...
The \googleVis package converts data frame into JSON objects.
...
to be encapsulate by a double backslash.

Here is a little example, see the output in Figure~\ref{fig:apostrophes}.
<<>>=
df <- data.frame("Year"=c(2010,2011), "Lloyd\\'s"=c(86.1, 93.3), 
