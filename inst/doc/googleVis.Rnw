...
\begin{abstract}
  ...
  browser via the R HTTP help server. 
...
\begin{itemize}
\item references to JavaScript functions provided by Google.
...
motion chart, line chart, bar chart, column chart, area chart,
scatter chart, combo chart, pie chart, geo chart, candlestick chart,
and org chart, further annotated time line, map,
geo map, table, tree map, intensity map, and
gauge~\cite{GoogleVisApi}. See Figure~\ref{demos} for some examples,
for more see the  
...
the output using the R HTTP help web server locally.  
...
Suppose you would like to show more than one interactive chart on the 
same page. 

The function \texttt{gvisMerge} takes two \texttt{gvis}-objects 
and merges the underlying components into one to align the charts either 
...
\subsection{Use \texttt{gvis} objects in blogs}

It is easy to post your results in a blog.
For example, the blog software \href{http://wordpress.org/}{wordpress} 
needs little effort to be set up. 
After installing the software, you only need the additional
plugin \href{http://wordpress.org/extend/plugins/custom-fields-shortcode/}{custom fields shortcode};
in fact, this plugin is one of many ways to insert javascript inside a post.
You may copy your \texttt{gvis} object to your clipboard in the following way:
<<eval=FALSE>>=
M <- gvisMotionChart(Fruits, "Fruit", "Year",
		     options=list(width=400, height=370))
writeClipboard(M$html$chart)
@
While working on the article, create a custom field with
for instance name \texttt{Fruits} and paste your clipboard inside the value textarea.
Inside the article, add \texttt{[cf]Fruits[/cf]} where your Motion Chart should appear.
That's all.
