...
formated as a \texttt{DataTable} plus further options.
...
The design of the corresponding functions is always similar, for a
Motion Chart we have: 
\begin{verbatim}
...
time variable for the plot, while options is a list of the various
plotting options for the motion chart, as specified in the Google
Visualisation API. 
...
code as list separated into Header, Chart, Caption and Footer.
