...
\subsection{Use select event with Listener}

The parameter \texttt{gvis.listener.jscode} embeds code inside
a select event listener. This parameter is available in
\texttt{gvisTable} and \texttt{gvisGeoMap}. 
It is possible to add javascript code and access the build in
method \texttt{getSelection()}.
...
<<>>=
  ...
  J1 <- gvisGeoMap(Exports, locationvar='Country', numvar='Profit',
    ...
    jscode <- "
       ...
       alert(text);
"
   J2 <- gvisTable(Population,options(gvis.listener.jscode=jscode))
...
For more details see \url{http://code.google.com/intl/de-DE/apis/chart/interactive/docs/reference.html#events}.
