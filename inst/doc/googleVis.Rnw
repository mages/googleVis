...
work and one has to use an indirect approach. This means that the \googleVis
chart has to be hosted on a separate page and is embedded into the blog
post by using the \texttt{<iframe>} tag, e.g.:
\begin{verbatim}
