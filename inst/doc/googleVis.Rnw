...
\begin{abstract}
  ...
  and display them either via the \rsp package or
...
web server in a browser with Internet connection and Flash; it may not
...
@ 
Please note that by default the visualisation output files will be
written into the \texttt{rsp/myAnlysis} folder of the \googleVis
library. The following  R-command will show the path to this folder: 
<<eval=FALSE>>=
...
folder, via   \texttt{install.packages("googleVis", lib=Sys.getenv("R\_LIBS\_USER"))} .
Please see also the \href{http://cran.r-project.org/doc/manuals/R-admin.html#Managing-libraries}{R Admin Manual}
or contact your system administrator if you need further help.
...
see page~\pageref{rspexample}, or Google Gadgets.  
...
default automatically create a rsp-file in the \texttt{rsp/myAnalysis}
folder of the \googleVis package using the type and chart id
information of the object and it will display the output using the
local web server of the \rsp package. 

The following R-command will show you the path to the
\texttt{rsp/myAnalysis} folder:
<<eval=FALSE>>=
system.file(file.path("rsp", "myAnalysis"), package="googleVis")
@ 
If you would like to write the output into a different directory,
e.g. your web server repository, then you can set the arguments for 
\texttt{filename} and \texttt{repos} (web server address of the
...
As mentioned above the R.rsp package allows us to dynamically
