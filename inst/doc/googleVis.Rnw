...
It exposes the following two methods: 
\begin{itemize}
...
into a gvis-object as a (undocumented) parameter
\texttt{gvis.listener.jscode} of the options list. 
...
plot(J1)
@ 
In the same way we could use the code in other charts, e.g. org- or line chart:                 
<<eval=FALSE>>=
