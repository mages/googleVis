...
formated as a \texttt{DataTable} and further optional settings for the plot.
...
\subsection{Using \texttt{googlvis} with \texttt{rsp}}
...
The demo \texttt{demo(googleVis)} demonstrates how several charts can
