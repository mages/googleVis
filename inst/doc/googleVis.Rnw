...
\newcommand{\googleVisVersion}{0.2.8}
...
Tree Maps, further Line-, Bar-, Column-, Area-, Combo-, Scatter-, Candlestick-,
Pie- and Org Charts; see Figure~\ref{demos} for some examples
for more see the  \href{http://code.google.com/p/google-motion-charts-with-r/wiki/GadgetExamples}{project
...
Please note that Flash charts may not not
work when loaded as a local file due to security settings, and
therefore require to be displayed via a web server. 
%% For more details see the Google Visualisation API documentation. 
...
options, see the output in Figure~\ref{Options}. 
\begin{figure}%%[!ht]
\begin{center}
...
A special option for all charts is \texttt{gvis.editor}, which puts a
button onto the page allowing the user to edit, change and customise the chart in
a browser window, see the following example and Figure~\ref{Editor}.
The content of the list item \texttt{gvis.editor} describes the label
of the browser button\footnote{See also \url{http://code.google.com/apis/chart/interactive/docs/reference.html\#google\_visualization\_charteditor}}.
<<eval=FALSE>>=
...
Gadget technology take from \href{http://en.wikipedia.org/wiki/Google_Gadgets}{Wikipedia}.
\begin{verbatim}
...
\item in Blogger via the design tab,
...
The 'addListner' method expects JavaScript code, which can be embedded
...
plot(J2)
@ 
For more details see the \texttt{demo(EventListner)} and \href{http://code.google.com/apis/chart/interactive/docs/reference.html}{Google Visualisation API Reference}.
