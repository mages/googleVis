...
Table~\cite{GoogleTable} and Tree Map~\cite{GoogleTreeMap}, see
Figure~\ref{demos} for examples.  
...
\rsp packages, so we need to install those as well.  Currently the \rjsonio
...
\googleVis in the usual way from CRAN, e.g.:
\begin{verbatim}
R> install.packages(c('R.rsp', 'googleVis'), 
                    repos='http://cran.r-project.org') 
\end{verbatim}

The installation of the \googleVis package was successful if the
...
time variable for the plot, while plotting options are set in an
...
footer. This allows either to create a complete html page, which the
default print function is doing with an \texttt{unlist} statement:
<<>>=
googleVis:::print.gvis
@ 
Or we can access the components individually. 
...
Further examples are part of the \googleVis demo and include one
example how the output of several visualisation can
be incorporated into a single page.


\subsection{Using \texttt{googlvis} with \texttt{rsp}}\label{rspexample}
...
As mentioned above the R.rsp packages
allows us to dynamically generate documents into static content using
R Server Pages. This means we can mix html and R code to create
content on the fly.   As an example, we can embed the above motion chart
into a rsp-page: 
\begin{verbatim}
<html>
<body>
