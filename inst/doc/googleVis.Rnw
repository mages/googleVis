...
We also noticed that data journalism has grown over the recent
...
API. The output of \googleVis functions is html code that contains the
data and references to Google JavaScript functions. To view the output
a browser with Flash and Internet  connection 
is required, the actual chart a rendered in the browser.
...
folder). Please contact your system administrator first if you need help. 
...
complete web page and at the same time can extract specific parts,
...
@
The structural output of \texttt{gvisMotionChart} is a list of list as described above
<<eval=FALSE>>=
 str(M)
@ 
<<echo=FALSE>>=
...
Further examples as part of the \googleVis demo, including one
example demonstrating how the output of several visualisations can
be incorporated into a single page.

\subsection{Using \texttt{google{V}is} with \texttt{rsp}}\label{rspexample}
...
\end{verbatim}
You find a few examples a part of the \googleVis package. Those examples can be displayed
