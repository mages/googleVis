...
\begin{abstract}
  ...
  data and references to JavaScript functions hosted by
  Google and can be displayed locally with the R HTTP
  help server.
...
The header part of the html page has only basic html tags: 
<<>>=
cat(M$html$header)
@ 
...
default automatically create a html file in a temporary folder using
the type and chart id information of the object and it will display
the output using the R HTTP web server.  The R command
\texttt{tempdir()}  will show you the path of the per-session
temporary directory.  
...
The \rsp package allows us to dynamically
...
\texttt{<\%=$\dots$\%>}, which acts as a \texttt{cat} statement. In the 
example above the chart code is embedded into the html code.

You find an example as part of the \googleVis package. This example can be displayed
...
would like to mix R and html code with we also need a parser and
...
two tasks are splited. This has the advantage that R does't have to
...
the \brew directory and access the via
