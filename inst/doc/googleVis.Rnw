...
\begin{abstract}
  ...
  motion chart, line chart, bar chart, column chart, area chart, combo
  chart,  scatter chart, pie chart, and org chart, further annotated time
  line, map, geo map, geo chart, table, tree map, intensity map, and gauge. 
\end{abstract}
...
scatter chart, , combo chart, pie chart, geo chart
and org chart, further annotated time line, map, geo map, table, tree
map, intensity map, and gauge~\cite{GoogleVisApi}.
See Figure~\ref{demos} for some examples, for more see the 
...
The function \texttt{gvisMerge} just does this.  

\texttt{gvisMerge} takes two \texttt{gvis}-objects and merges the underlying
components into one to align the charts either horizontally or vertically next to
each other in an HTML table. 
...
The following example, see Figure~\ref{gvisMerge}, aligns a geo chart
and table below each other, and combines this output with a motion chart to the right:
<<>>=
