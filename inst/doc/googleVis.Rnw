...
\newcommand{\googleVisVersion}{0.2.9}
...
\item in Blogger via the design tab, for site gadget
...
Situation is again different if you want to include the \googleVis
output into a blog post.  Here a gadget doesn't seem to work and an
\texttt{<iframe>} tag has to be used. With the 
